...
bool output_ann(struct whisper_context * ctx, const char * fname) {
    ...
    fprintf(stderr, "%s: saving output to '%s'\n", __func__, fname);
    fprintf(fp, "segment_id\ttoken_type\ttoken_id\tcontent\ttoken_probability\ttoken_number\tstart_time\tend_time\tduration\ttoken_vlen\ttoken_prob_timestamp\ttoken_prob_timestamp_sum\n");
    ...
    for (int i = 0; i < whisper_full_n_segments(ctx); i++) {
        const int64_t t0 = whisper_full_get_segment_t0(ctx, i);
        const int64_t t1 = whisper_full_get_segment_t1(ctx, i);
        const int64_t segment_length = t1-t0;
        ...
        for (int j = 0; j < n; ++j) {
            ...
            const char * text = whisper_full_get_token_text(ctx, i, j);
            const int64_t segment_length =tokens[j].t1-tokens[j].t0;
            const whisper_token id = whisper_full_get_token_id(ctx, i, j);

            // token_type = (id >= whisper_token_eot(ctx)) ? "special" : "pronuntiation";
            if (id >= whisper_token_eot(ctx)) {
                token_type="special";
            } else if ((id==11)||(id==13)||(id==30)) {
                token_type="punctuation";
            } else {
                token_type="pronuntiation";
            }
            token_id+=1;
            fprintf(fp, "%5d\t%s\t%d\t%s\t%6.3f\t%5d\t%s\t%s\t%s\t%6.3f\t%6.3f\t%6.3f\n",
                segment_id,token_type.c_str(),id,text,tokens[j].p,token_id,
                to_timestamp(tokens[j].t0).c_str(), 
                to_timestamp(tokens[j].t1).c_str(), to_timestamp(segment_length).c_str(),
                tokens[j].vlen, tokens[j].pt, tokens[j].ptsum);
            if (id==13) {segment_id+=1;  token_id=0;}
        }
    }
